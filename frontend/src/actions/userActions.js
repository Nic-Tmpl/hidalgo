...
const login = (email, password) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`${LOCAL_API_URL}/login/password`, { username: email, password: password });
...
const logout = () => async (dispatch) => {
    const { data } = await axios.post(`${LOCAL_API_URL}/logout`);
...
const userEdit = (id, email, password, firstName, lastName) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.put(`${LOCAL_API_URL}/users/${id}`, {
            email: email,
            password: password,
...
const signup = (email, password, firstName, lastName) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`${LOCAL_API_URL}/signup`, {
            email: email, 
            password: password,
            first_name: firstName,
            last_name: lastName,
